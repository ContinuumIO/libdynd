...
namespace nd {
  ...
  class not_equal_callable : public default_instantiable_callable<not_equal_kernel<Arg0ID, Arg1ID>> {
  public:
    not_equal_callable()
        : default_instantiable_callable<not_equal_kernel<Arg0ID, Arg1ID>>(
  ...
  public:
    struct not_equal_call_frame : call_frame {
    ...
    void new_resolve(base_callable *DYND_UNUSED(parent), call_graph &cg, ndt::type &dst_tp, intptr_t nsrc,
      ...
      not_equal_call_frame *frame = reinterpret_cast<not_equal_call_frame *>(cg.back());
      ...
      for (size_t i = 0; i != field_count; ++i) {
        ...
        if (!not_equal->is_abstract()) {
          cg.emplace_back(not_equal.get());
        }
        not_equal->new_resolve(this, cg, dst_tp, nsrc, child_src_tp, nkwd, kwds, tp_vars);
      }
    }
    ...
    void new_instantiate(call_frame *frame, kernel_builder &ckb, kernel_request_t kernreq, const char *dst_arrmeta,
      ...
      size_t field_count = reinterpret_cast<not_equal_call_frame *>(frame)->arrmeta_offsets.size();
      ...
      not_equal_kernel<tuple_id, tuple_id> *self = ckb.get_at<not_equal_kernel<tuple_id, tuple_id>>(self_offset);
      const std::vector<uintptr_t> &arrmeta_offsets = reinterpret_cast<not_equal_call_frame *>(frame)->arrmeta_offsets;
      for (size_t i = 0; i != field_count; ++i) {
        self = ckb.get_at<not_equal_kernel<tuple_id, tuple_id>>(self_offset);
        size_t *field_kernel_offsets = reinterpret_cast<size_t *>(self + 1);
