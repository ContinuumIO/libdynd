...
*/
  ...
  struct expression_assignment_kernel : base_virtual_kernel<expression_assignment_kernel> {
    static intptr_t instantiate(char *DYND_UNUSED(static_data), char *DYND_UNUSED(data), void *ckb, intptr_t ckb_offset,
                                const ndt::type &dst_tp, const char *dst_arrmeta, intptr_t DYND_UNUSED(nsrc),
                                const ndt::type *src_tp, const char *const *src_arrmeta, kernel_request_t kernreq,
                                const eval::eval_context *ectx, intptr_t DYND_UNUSED(nkwd),
                                ...
                                const std::map<std::string, ndt::type> &DYND_UNUSED(tp_vars))
    {
      return make_expression_assignment_kernel(ckb, ckb_offset, dst_tp, dst_arrmeta, src_tp[0], src_arrmeta[0], kernreq,
                                               ectx);
    }
  };
...
namespace ndt {
  ...
  template <>
  struct type::equivalent<nd::expression_assignment_kernel> {
    static type make() { return ndt::type("(Any) -> Any"); }
  };
