  ...
  namespace functional {
    ...
    struct elwise_virtual_ck : base_virtual_kernel<elwise_virtual_ck<N>> {
      ...
      static intptr_t instantiate(char *static_data, char *data, void *ckb, intptr_t ckb_offset,
                                  ...
                                  dynd::kernel_request_t kernreq, kernel_targets_t *DYND_UNUSED(targets),
        ...
        if (dst_ndim == 0) {
          ...
          if (i == nsrc) {
            ...
            return child.get()->instantiate(child.get()->static_data(), NULL, ckb, ckb_offset, dst_tp, dst_arrmeta,
                                            nsrc, src_tp, src_arrmeta, kernreq, NULL, ectx, nkwd, kwds, tp_vars);
          }
          else {
            intptr_t src_ndim = src_tp[i].get_ndim() - child_tp->get_pos_type(i).get_ndim();
            std::stringstream ss;
        ...
        case fixed_dim_type_id:
          if (src_all_strided) {
            return elwise_ck<fixed_dim_type_id, fixed_dim_type_id, N>::instantiate(
                static_data, data, ckb, ckb_offset, dst_tp, dst_arrmeta, nsrc, src_tp, src_arrmeta, kernreq, NULL, ectx,
                nkwd, kwds, tp_vars);
          }
                                ...
                                const nd::array *kwds, const std::map<std::string, ndt::type> &tp_vars)
      {
        ...
        if (!finished) {
          return nd::functional::elwise_virtual_ck<N>::instantiate(
              ...
              child_src_arrmeta, kernreq, NULL, ectx, nkwd, kwds, tp_vars);
        }

        // Instantiate the elementwise handler
        return child.get()->instantiate(child.get()->static_data(), NULL, ckb, ckb_offset, child_dst_tp,
                                        child_dst_arrmeta, nsrc, child_src_tp, child_src_arrmeta, kernreq, NULL, ectx,
                                        nkwd, kwds, tp_vars);
      }
    };
                                ...
                                const nd::array *kwds, const std::map<std::string, ndt::type> &tp_vars)
      {
        ...
        if (!finished) {
          return nd::functional::elwise_virtual_ck<0>::instantiate(static_data, data, ckb, ckb_offset, child_dst_tp,
                                                                   child_dst_arrmeta, nsrc, NULL, NULL, kernreq, NULL,
                                                                   ectx, nkwd, kwds, tp_vars);
        }

        // Instantiate the elementwise handler
        return child.get()->instantiate(child.get()->static_data(), NULL, ckb, ckb_offset, child_dst_tp,
                                        child_dst_arrmeta, nsrc, NULL, NULL, kernreq, NULL, ectx, nkwd, kwds, tp_vars);
      }
    };
    ...
    struct elwise_ck<fixed_dim_type_id, var_dim_type_id, N>
                                ...
                                const nd::array *kwds, const std::map<std::string, ndt::type> &tp_vars)
      {
        ...
        if (!finished) {
          return nd::functional::elwise_virtual_ck<N>::instantiate(
              static_data, data, ckb, ckb_offset, child_dst_tp, child_dst_arrmeta, nsrc, child_src_tp,
              child_src_arrmeta, kernel_request_strided, NULL, ectx, nkwd, kwds, tp_vars);
        }
        // Instantiate the elementwise handler
        return child.get()->instantiate(child.get()->static_data(), NULL, ckb, ckb_offset, child_dst_tp,
                                        child_dst_arrmeta, nsrc, child_src_tp, child_src_arrmeta,
                                        kernel_request_strided, &child.get()->targets, ectx, nkwd, kwds, tp_vars);
      }
    };
                                ...
                                const nd::array *kwds, const std::map<std::string, ndt::type> &tp_vars)
      {
        ...
        if (!finished) {
          return nd::functional::elwise_virtual_ck<0>::instantiate(
              static_data, data, ckb, ckb_offset, child_dst_tp, child_dst_arrmeta, nsrc, NULL, NULL,
              kernel_request_strided, NULL, ectx, nkwd, kwds, tp_vars);
        }
        // Instantiate the elementwise handler
        return child.get()->instantiate(child.get()->static_data(), NULL, ckb, ckb_offset, child_dst_tp,
                                        child_dst_arrmeta, nsrc, NULL, NULL, kernel_request_strided,
                                        &child.get()->targets, ectx, nkwd, kwds, tp_vars);
      }
    };
