...
#include <dynd/kernels/variadic.hpp>
...
namespace nd {
  namespace functional {
    ...
    template <int N>
    struct elwise_virtual_ck : virtual_ck<elwise_virtual_ck<N>> {
      static void
      ...
      {
        const arrfunc_type_data *child = self->get_data_as<arrfunc>()->get();
        const arrfunc_type *child_tp = self->get_data_as<arrfunc>()->get_type();
                       ...
                       const std::map<dynd::nd::string, ndt::type> &tp_vars)
      {
        ...
        for (intptr_t i = 0; i < nsrc; ++i) {
          intptr_t child_ndim_i = child_tp->get_pos_type(i).get_ndim();
        ...
        child->resolve_dst_type(
            child, child_tp, NULL, child_dst_tp, nsrc,
        ...
        if (dst_ndim == 0) {
          ...
          if (i == src_count) {
            ...
            intptr_t src_ndim =
                src_tp[i].get_ndim() - child_tp->get_pos_type(i).get_ndim();
            std::stringstream ss;
            ss << "Trying to broadcast " << src_ndim << " dimensions of "
    ...
    template <int N>
    struct elwise_ck<fixed_dim_type_id, fixed_dim_type_id, N, 0>
        : expr_ck<elwise_ck<fixed_dim_type_id, fixed_dim_type_id, N, 0>,
    ...
    template <int N>
    struct elwise_ck<fixed_dim_type_id, fixed_dim_type_id, N, -1>
        : virtual_ck<elwise_ck<fixed_dim_type_id, fixed_dim_type_id, N, -1>> {
      typedef elwise_ck self_type;
                  ...
                  const std::map<dynd::nd::string, ndt::type> &tp_vars)
      {
        ...
        elwise_ck<fixed_dim_type_id, fixed_dim_type_id, N, 0>::create(
            ckb, kernreq, ckb_offset, size, dst_stride,
            dynd::detail::make_array_wrapper<N>(src_stride));
        kernreq = (kernreq & kernel_request_memory) | kernel_request_strided;
        ...
        if (!finished) {
          return nd::functional::elwise_virtual_ck<N>::instantiate(
              self, self_tp, NULL, ckb, ckb_offset, child_dst_tp,
    ...
    template <>
    struct elwise_ck<fixed_dim_type_id, fixed_dim_type_id, 0, -1>
        : expr_ck<elwise_ck<fixed_dim_type_id, fixed_dim_type_id, 0, -1>,
     ...
     */
    template <int N>
    struct elwise_ck<fixed_dim_type_id, var_dim_type_id, N, -1>
        : expr_ck<elwise_ck<fixed_dim_type_id, var_dim_type_id, N, -1>,
    ...
    template <>
    struct elwise_ck<fixed_dim_type_id, var_dim_type_id, 0, -1>
        : expr_ck<elwise_ck<fixed_dim_type_id, var_dim_type_id, 0, -1>,
     ...
     */
    template <int N>
    struct elwise_ck<var_dim_type_id, fixed_dim_type_id, N, -1>
        : expr_ck<elwise_ck<var_dim_type_id, fixed_dim_type_id, N, -1>,
                  kernel_request_host, N> {
      typedef elwise_ck self_type;
    ...
    template <>
    struct elwise_ck<var_dim_type_id, fixed_dim_type_id, 0, -1>
        : expr_ck<elwise_ck<var_dim_type_id, fixed_dim_type_id, 0, -1>,
                  kernel_request_host, 0> {
      typedef elwise_ck self_type;
    ...
    template <int N>
    struct elwise_ck<var_dim_type_id, var_dim_type_id, N, -1>
        : elwise_ck<var_dim_type_id, fixed_dim_type_id, N, -1> {
    };
