...
namespace nd {
  namespace detail {
    ...
    struct is_avail_kernel<Src0TypeID, sint_kind>
      ...
      void single(char *dst, char *const *src)
      {
        *dst = **reinterpret_cast<A0 *const *>(src) !=
               std::numeric_limits<A0>::min();
      }
                   ...
                   const intptr_t *src_stride, size_t count)
      {
        ...
        for (size_t i = 0; i != count; ++i) {
          *dst =
              *reinterpret_cast<A0 *>(src0) != std::numeric_limits<A0>::min();
          dst += dst_stride;
          src0 += src0_stride;
        }
      }
    ...
    struct is_avail_kernel<complex_float32_type_id, complex_kind>
                   ...
                   const intptr_t *src_stride, size_t count)
      {
        ...
        for (size_t i = 0; i != count; ++i) {
          *dst =
              reinterpret_cast<uint32_t *>(src0)[0] !=
                  DYND_FLOAT32_NA_AS_UINT &&
    ...
    struct is_avail_kernel<complex_float64_type_id, complex_kind>
                   ...
                   const intptr_t *src_stride, size_t count)
      {
        ...
        for (size_t i = 0; i != count; ++i) {
          *dst =
              reinterpret_cast<uint64_t *>(src0)[0] !=
                  DYND_FLOAT64_NA_AS_UINT &&
    ...
    struct is_avail_kernel<fixed_dim_type_id, dim_kind>
          ...
          const std::map<nd::string, ndt::type> &DYND_UNUSED(tp_vars))
      {
        ...
        case bool_type_id:
          is_avail_kernel<bool_type_id, bool_kind>::make(ckb, kernreq,
                                                         ckb_offset);
          return ckb_offset;
        case int8_type_id:
          is_avail_kernel<int8_type_id, sint_kind>::make(ckb, kernreq,
                                                         ckb_offset);
          return ckb_offset;
        case int16_type_id:
          is_avail_kernel<int16_type_id, sint_kind>::make(ckb, kernreq,
                                                          ckb_offset);
          return ckb_offset;
        case int32_type_id:
          is_avail_kernel<int32_type_id, sint_kind>::make(ckb, kernreq,
                                                          ckb_offset);
          return ckb_offset;
        case int64_type_id:
          is_avail_kernel<int64_type_id, sint_kind>::make(ckb, kernreq,
                                                          ckb_offset);
          return ckb_offset;
        case int128_type_id:
          is_avail_kernel<int128_type_id, sint_kind>::make(ckb, kernreq,
                                                           ckb_offset);
          return ckb_offset;
        case float32_type_id:
          is_avail_kernel<float32_type_id, real_kind>::make(ckb, kernreq,
                                                            ckb_offset);
          return ckb_offset;
        case float64_type_id:
          is_avail_kernel<float64_type_id, real_kind>::make(ckb, kernreq,
                                                            ckb_offset);
          return ckb_offset;
        case complex_float32_type_id:
          is_avail_kernel<complex_float32_type_id, complex_kind>::make(
              ckb, kernreq, ckb_offset);
          return ckb_offset;
        case complex_float64_type_id:
          is_avail_kernel<complex_float64_type_id, complex_kind>::make(
              ckb, kernreq, ckb_offset);
          return ckb_offset;
        default:
