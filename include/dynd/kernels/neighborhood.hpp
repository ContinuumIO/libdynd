    ...
    struct neighborhood_kernel : base_kernel<neighborhood_kernel<N>, N> {
                                  ...
                                  const nd::array *kwds, const std::map<std::string, ndt::type> &tp_vars)
      {
        ...
        if (reinterpret_cast<data_type *>(data)->ndim == 0) {
          ...
          ckb_offset = child.get()->instantiate(child.get()->static_data(), NULL, ckb, ckb_offset, child_dst_tp,
                                                child_dst_arrmeta, nsrc,
                                                &reinterpret_cast<data_type *>(data)->child_src_tp, &child_src_arrmeta,
                                                kernel_request_single, NULL, ectx, nkwd - 3, kwds + 3, tp_vars);
          ...
          ckb_offset = boundary_child.get()->instantiate(
              boundary_child.get()->static_data(), NULL, ckb, ckb_offset, child_dst_tp, child_dst_arrmeta, 0, NULL,
              NULL, kernel_request_single, NULL, ectx, nkwd - 3, kwds + 3, tp_vars);
        ...
        return instantiate(static_data, data, ckb, ckb_offset, child_dst_tp, child_dst_arrmeta, nsrc, child_src_tp,
                           child_src_arrmeta, kernel_request_single, NULL, ectx, nkwd, kwds, tp_vars);
      }
    };
