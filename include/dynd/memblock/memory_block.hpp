...
struct DYND_API memory_block_data {
  /**
   * This is a struct of function pointers for allocating and resizing data.
   */
  struct DYND_API api {
    /**
...
public:
  /** Default constructor */
  memory_block_ptr() : m_memblock(0)
  {
  }

  /** Constructor from a raw pointer */
  explicit memory_block_ptr(memory_block_data *memblock, bool add_ref = true) : m_memblock(memblock)
  {
  ...
  /** Copy constructor */
  memory_block_ptr(const memory_block_ptr &rhs) : m_memblock(rhs.m_memblock)
  {
    if (m_memblock != 0) {
  ...
  /** Move constructor */
  memory_block_ptr(memory_block_ptr &&rhs) : m_memblock(rhs.m_memblock)
  {
    rhs.m_memblock = 0;
  }
