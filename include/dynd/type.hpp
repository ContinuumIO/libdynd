...
// Forward declaration of strided_vals
template <typename T>
class fixed_dim;

// Forward declaration of nd::array
namespace nd {
  class array;
} // namespace nd
...
namespace ndt {
  ...
  type make_fixed_dimsym(const type &element_tp, intptr_t size);
  ...
  namespace detail {
    ...
    template <typename T>
    struct type_from<dynd::fixed_dim<T>> {
      static type make() { return make_fixed_dimsym(type_from<T>::make(), 1); }
    };
