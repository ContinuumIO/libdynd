...
public:
  ...
  /**
   * Modifies arrmeta allocated using the arrmeta_default_construct function, to
   *be used
   * immediately after nd::array construction. Given an input type/arrmeta,
   *edits the output
   * arrmeta in place to match.
   *
   * \param dst_arrmeta  The arrmeta created by arrmeta_default_construct, which
   *is modified in place
   * \param src_tp  The type of the input nd::array whose stride ordering is to
   *be matched.
...
namespace ndt {
  inline ndt::type make_fixed_dim(size_t dim_size) {
    return make_fixed_dim(dim_size, ndt::type());
  }
