...
namespace ndt {
  ...
  protected:
    ...
    tuple_type(type_id_t type_id, std::vector<type> field_types, flags_type flags, bool layout_in_arrmeta,
               bool variadic);

    uintptr_t *get_arrmeta_data_offsets(char *arrmeta) const { return reinterpret_cast<uintptr_t *>(arrmeta); }

  public:
    tuple_type(std::vector<type> field_types, bool variadic);
    ...
    std::vector<type> get_field_types() const { return m_field_types; }
    nd::array field_types_as_ndarray() const { return m_field_types; }

    const type *get_field_types_raw() const { return &m_field_types[0]; }
    /** The array of the field arrmeta offsets */
    nd::array get_arrmeta_offsets() const { return m_arrmeta_offsets; }
