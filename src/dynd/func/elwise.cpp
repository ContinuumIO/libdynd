...
ndt::type nd::functional::elwise_make_type(const ndt::callable_type *child_tp)
{
  ...
  for (const ndt::type &t : param_types) {
    if (t.get_kind() == memory_kind) {
      out.push_back(ndt::make_ellipsis_dim(dimsname, t.without_memory_type()));
    }
    else if (t.get_id() == typevar_constructed_id) {
      ...
      out.push_back(ndt::make_ellipsis_dim(dimsname, t));
    }
  }
