    ...
    const eval::eval_context *ectx)
{
  ...
  for (intptr_t i = 0; i < field_count; ++i) {
    self = tuple_unary_op_ck::reserve(ckb, kernreq, root_ckb_offset, ckb_offset);
    ...
    ckb_offset = af->instantiate(af, af_tp, NULL, ckb, ckb_offset, dst_tp[i],
    ...
    kernel_request_t kernreq, const eval::eval_context *ectx)
{
  ...
  for (intptr_t i = 0; i < field_count; ++i) {
    ...
    ckb_offset = af[i]->instantiate(
        af[i], af_tp[i], NULL, ckb, ckb_offset, dst_tp[i], dst_arrmeta[i], 1,
    ...
    const eval::eval_context *ectx)
{
  ...
  auto sd = val_tup_tp.extended<base_tuple_type>();
                                          ...
                                          const eval::eval_context *ectx)
{
  ...
  auto dst_sd = dst_tuple_tp.extended<base_tuple_type>();
  auto src_sd = src_tuple_tp.extended<base_tuple_type>();
