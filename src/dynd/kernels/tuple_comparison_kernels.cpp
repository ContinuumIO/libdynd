...
struct tuple_compare_sorting_less_matching_arrmeta_kernel {
  ...
  static void sorting_less(ckernel_prefix *extra, char *dst, char *const *src)
  {
    ...
    for (size_t i = 0; i != field_count; ++i) {
      ckernel_prefix *sorting_less_kdp = reinterpret_cast<ckernel_prefix *>(eraw + kernel_offsets[i]);
      ...
      int child_dst;
      opchild(sorting_less_kdp, reinterpret_cast<char *>(&child_dst), child_src);
      if (child_dst) {
...
struct tuple_compare_sorting_less_diff_arrmeta_kernel {
  ...
  static void sorting_less(ckernel_prefix *extra, char *dst, char *const *src)
  {
    ...
    char *child_src[2];
    for (size_t i = 0; i != field_count; ++i) {
      ckernel_prefix *src0_sorting_less_src1 = reinterpret_cast<ckernel_prefix *>(eraw + kernel_offsets[2 * i]);
      expr_single_t opchild = src0_sorting_less_src1->get_function<expr_single_t>();
      // if (src0.field_i < src1.field_i) return true
      child_src[0] = src[0] + src0_data_offsets[i];
      child_src[1] = src[1] + src1_data_offsets[i];
      int child_dst;
      opchild(src0_sorting_less_src1, reinterpret_cast<char *>(&child_dst), child_src);
      if (child_dst) {
        *reinterpret_cast<char *>(dst) = true;
        return;
      }
      ckernel_prefix *src1_sorting_less_src0 = reinterpret_cast<ckernel_prefix *>(eraw + kernel_offsets[2 * i + 1]);
      ...
      opchild(src1_sorting_less_src0, reinterpret_cast<char *>(&child_dst), child_src);
      if (child_dst) {
  ...
  if (comptype == comparison_type_sorting_less) {
    ...
    if (comptype == comparison_type_equal) {
      ...
      return nd::equal_kernel<tuple_type_id, tuple_type_id>::instantiate(
          NULL, NULL, ckb, ckb_offset, ndt::type::make<bool1>(), NULL, 2, &src_tp, src_arrmeta,
          kernel_request_host | kernel_request_single, NULL, ectx, 0, NULL, tp_vars);
    }
    else {
      std::map<std::string, ndt::type> tp_vars;
      const char *src_arrmeta[2] = {src0_arrmeta, src1_arrmeta};
