                           ...
                           const char *src_data)
{
  ...
  if (tp.is_pod()) {
    ...
    make_assignment_kernel(&k, 0, tp, dst_arrmeta, tp, src_arrmeta, kernel_request_single, &eval::default_eval_context);
                             ...
                             const char *src_arrmeta, const char *src_data, const eval::eval_context *ectx)
{
  ...
  if (ectx == &eval::default_eval_context) {
    (*nd::assign::get().get())(dst_tp, dst_arrmeta, dst_data, 1, &src_tp, &src_arrmeta,
                               const_cast<char *const *>(&src_data), 0, NULL, std::map<std::string, ndt::type>());
    return;
  }

  ckernel_builder<kernel_request_host> k;
  make_assignment_kernel(&k, 0, dst_tp, dst_arrmeta, src_tp, src_arrmeta, kernel_request_single, ectx);
  expr_single_t fn = k.get()->get_function<expr_single_t>();
  char *src = const_cast<char *>(src_data);
  fn(k.get(), dst_data, &src);
}
