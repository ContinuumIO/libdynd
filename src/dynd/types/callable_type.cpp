...
std::map<std::string, nd::callable> ndt::callable_type::get_dynamic_type_properties() const
{
  ...
  properties["pos_types"] = nd::callable::make<nd::get_then_copy_kernel2<callable_type, &callable_type::pos_types_as_ndarray>>(
      ndt::callable_type::make(m_pos_tuple.extended<ndt::tuple_type>()->field_types_as_ndarray().get_type(),
                               ndt::tuple_type::make(),
                               ndt::struct_type::make({"self"}, {ndt::make_type<ndt::type_type>()})));
  properties["kwd_types"] = nd::callable::make<nd::get_then_copy_kernel2<callable_type, &callable_type::kwd_types_as_ndarray>>(
      ndt::callable_type::make(kwd_types_as_ndarray().get_type(),
                               ndt::tuple_type::make(),
                               ndt::struct_type::make({"self"}, {ndt::make_type<ndt::type_type>()})));
  properties["kwd_names"] = nd::callable::make<nd::get_then_copy_kernel2<callable_type, &callable_type::get_kwd_names>>(
