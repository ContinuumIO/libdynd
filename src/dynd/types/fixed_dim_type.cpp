...
ndt::fixed_dim_type::fixed_dim_type(intptr_t dim_size, const type &element_tp)
    : base_dim_type(fixed_dim_type_id, element_tp, 0,
...
void ndt::fixed_dim_type::print_data(std::ostream &o, const char *arrmeta,
...
bool ndt::fixed_dim_type::is_c_contiguous(const char *arrmeta) const
{
  ...
  if (static_cast<intptr_t>(m_element_tp.get_default_data_size()) ==
      ss->stride) {
...
void ndt::fixed_dim_type::transform_child_types(
    type_transform_fn_t transform_fn, intptr_t arrmeta_offset, void *extra,
    type &out_transformed_tp, bool &out_was_transformed) const
{
  type tmp_tp;
  bool was_transformed = false;
  transform_fn(m_element_tp, arrmeta_offset + sizeof(fixed_dim_type_arrmeta),
...
{
  return type(new fixed_dim_type(m_dim_size, m_element_tp.get_canonical_type()),
              false);
}

ndt::type ndt::fixed_dim_type::apply_linear_index(intptr_t nindices,
                                                  ...
                                                  bool leading_dimension) const
{
  if (nindices == 0) {
    ...
    if (remove_dimension) {
      ...
      return make_fixed_dim(dimension_size, m_element_tp.apply_linear_index(
...
intptr_t ndt::fixed_dim_type::apply_linear_index(
    ...
    memory_block_data **inout_dataref) const
{
  const fixed_dim_type_arrmeta *md =
      reinterpret_cast<const fixed_dim_type_arrmeta *>(arrmeta);
  fixed_dim_type_arrmeta *out_md =
      reinterpret_cast<fixed_dim_type_arrmeta *>(out_arrmeta);
  if (nindices == 0) {
    ...
    if (remove_dimension) {
        ...
        if (leading_dimension) {
          ...
          *inout_data += offset;
          offset = m_element_tp.extended()->apply_linear_index(
              nindices - 1, indices + 1,
              arrmeta + sizeof(fixed_dim_type_arrmeta), result_tp, out_arrmeta,
              embedded_reference, current_i + 1, root_tp, true, inout_data,
              inout_dataref);
        } else {
          offset += m_element_tp.extended()->apply_linear_index(
              nindices - 1, indices + 1,
      ...
      if (!m_element_tp.is_builtin()) {
        ...
        offset += m_element_tp.extended()->apply_linear_index(
            nindices - 1, indices + 1, arrmeta + sizeof(fixed_dim_type_arrmeta),
            result_etp->get_element_type(),
            out_arrmeta + sizeof(fixed_dim_type_arrmeta), embedded_reference,
...
ndt::type ndt::fixed_dim_type::at_single(intptr_t i0,
...
ndt::type ndt::fixed_dim_type::get_type_at_dimension(char **inout_arrmeta,
                                                     intptr_t i,
...
intptr_t ndt::fixed_dim_type::get_dim_size(const char *DYND_UNUSED(arrmeta),
                                           const char *DYND_UNUSED(data)) const
{
  return m_dim_size;
}

void ndt::fixed_dim_type::get_shape(intptr_t ndim, intptr_t i,
...
void ndt::fixed_dim_type::get_strides(size_t i, intptr_t *out_strides,
...
ndt::fixed_dim_type::classify_axis_order(const char *arrmeta) const
{
  const fixed_dim_type_arrmeta *md =
      reinterpret_cast<const fixed_dim_type_arrmeta *>(arrmeta);
...
bool ndt::fixed_dim_type::is_lossless_assignment(const type &dst_tp,
...
void ndt::fixed_dim_type::arrmeta_default_construct(char *arrmeta,
...
void ndt::fixed_dim_type::arrmeta_copy_construct(
...
void ndt::fixed_dim_type::arrmeta_debug_print(const char *arrmeta,
                                              std::ostream &o,
...
size_t ndt::fixed_dim_type::get_iterdata_size(intptr_t ndim) const
{
  if (ndim == 0) {
    ...
    return m_element_tp.get_iterdata_size(ndim - 1) +
           sizeof(fixed_dim_type_iterdata);
  }
}
...
{
  fixed_dim_type_iterdata *id =
      reinterpret_cast<fixed_dim_type_iterdata *>(iterdata);
  if (level == 0) {
    id->data += id->stride;
    return id->data;
  } else {
...
static char *iterdata_reset(iterdata_common *iterdata, char *data,
                            intptr_t ndim)
{
  fixed_dim_type_iterdata *id =
      reinterpret_cast<fixed_dim_type_iterdata *>(iterdata);
...
size_t ndt::fixed_dim_type::iterdata_construct(iterdata_common *iterdata,
                                               const char **inout_arrmeta,
                                               intptr_t ndim,
                                               ...
                                               type &out_uniform_tp) const
{
  ...
  *inout_arrmeta += sizeof(fixed_dim_type_arrmeta);
  ...
  if (ndim > 1) {
    ...
    inner_size = m_element_tp.extended()->iterdata_construct(
        iterdata, inout_arrmeta, ndim - 1, shape + 1, out_uniform_tp);
    iterdata = reinterpret_cast<iterdata_common *>(
        reinterpret_cast<char *>(iterdata) + inner_size);
  } else {
...
size_t ndt::fixed_dim_type::iterdata_destruct(iterdata_common *iterdata,
                                              intptr_t ndim) const
{
  size_t inner_size = 0;
  if (ndim > 1) {
...
void ndt::fixed_dim_type::data_destruct(const char *arrmeta, char *data) const
{
  ...
  m_element_tp.extended()->data_destruct_strided(
      arrmeta + sizeof(fixed_dim_type_arrmeta), data, md->stride, m_dim_size);
}

void ndt::fixed_dim_type::data_destruct_strided(const char *arrmeta, char *data,
                                                intptr_t stride,
                                                size_t count) const
{
  const fixed_dim_type_arrmeta *md =
  ...
  for (size_t i = 0; i != count; ++i, data += stride) {
    m_element_tp.extended()->data_destruct_strided(arrmeta, data, child_stride,
                                                   child_size);
  }
}
...
void ndt::fixed_dim_type::reorder_default_constructed_strides(
    char *dst_arrmeta, const type &src_tp, const char *src_arrmeta) const
{
  ...
  if (ndim_partial < ndim && !last_src_tp.is_builtin()) {
    axis_order_classification_t aoc =
        last_src_tp.extended()->classify_axis_order(src_arrmeta);
    ...
    size_t *out_count) const
{
  ...
  *out_count =
      sizeof(fixed_dim_type_properties) / sizeof(fixed_dim_type_properties[0]);
}

void ndt::fixed_dim_type::get_dynamic_array_properties(
    const std::pair<std::string, gfunc::callable> **out_properties,
    size_t *out_count) const
{
...
void ndt::fixed_dim_type::get_dynamic_array_functions(
