    ...
    : base_string_type(fixed_string_type_id, 0, 1, type_flag_none, 0), m_stringsize(stringsize), m_encoding(encoding)
{
  ...
  case string_encoding_utf_8:
    data_size = m_stringsize;
    data_alignment = 1;
    break;
  case string_encoding_ucs_2:
  case string_encoding_utf_16:
    data_size = m_stringsize * 2;
    data_alignment = 2;
    break;
  case string_encoding_utf_32:
    data_size = m_stringsize * 4;
    data_alignment = 4;
    break;
  default:
...
intptr_t ndt::fixed_string_type::make_assignment_kernel(void *ckb, intptr_t ckb_offset, const type &dst_tp,
                                                        ...
                                                        const eval::eval_context *ectx) const
{
  if (this == dst_tp.extended()) {
    ...
    case fixed_string_type_id: {
      ...
      return make_fixed_string_assignment_kernel(ckb, ckb_offset, get_data_size(), m_encoding, src_fs->get_data_size(),
                                                 src_fs->m_encoding, kernreq, ectx);
    }
    case string_type_id: {
      const base_string_type *src_fs = src_tp.extended<base_string_type>();
      return make_blockref_string_to_fixed_string_assignment_kernel(ckb, ckb_offset, get_data_size(), m_encoding,
...
size_t ndt::fixed_string_type::make_comparison_kernel(void *ckb, intptr_t ckb_offset, const type &src0_dt,
