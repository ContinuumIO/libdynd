...
ndt::struct_type::struct_type(nd::array field_names, std::vector<type> field_types, bool variadic)
...
{
  std::vector<type> tmp_field_types;
  bool was_transformed = false;

  for (intptr_t i = 0; i < m_field_count; ++i) {
    tmp_field_types.push_back(make_type<type_type>());
  }
  ...
  if (nindices == 0) {
    ...
    if (remove_dimension) {
      ...
      for (intptr_t i = 0; i < dimension_size; ++i) {
        ...
        type t = get_field_type(idx).apply_linear_index(nindices - 1, indices + 1, current_i + 1, root_tp, false);
...
std::map<std::string, nd::callable> ndt::struct_type::get_dynamic_type_properties() const
{
  ...
  properties["field_types"] = nd::callable::make<nd::get_then_copy_kernel2<tuple_type, &tuple_type::field_types_as_ndarray>>(
      ndt::callable_type::make(field_types_as_ndarray().get_type(), ndt::tuple_type::make(),
                               ndt::struct_type::make({"self"}, {ndt::make_type<ndt::type_type>()})));
  properties["metadata_offsets"] =
      nd::callable::make<nd::get_then_copy_kernel2<tuple_type, &tuple_type::get_arrmeta_offsets>>(
          ndt::callable_type::make(m_arrmeta_offsets.get_type(), ndt::tuple_type::make(),
                                   ndt::struct_type::make({"self"}, {ndt::make_type<ndt::type_type>()})));
  properties["field_names"] = nd::callable::make<nd::get_then_copy_kernel2<struct_type, &struct_type::get_field_names>>(
