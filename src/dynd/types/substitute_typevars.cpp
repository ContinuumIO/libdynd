    ...
    bool concrete)
{
  ...
  case cuda_device_type_id:
    return ndt::make_cuda_device(ndt::substitute(
  ...
  case fixed_dim_type_id:
    if (pattern.get_kind() == kind_kind) {
      ...
      return ndt::make_fixed_dim(
          ...
          ndt::substitute(
              pattern.extended<fixed_dim_type>()->get_element_type(), typevars,
              concrete));
    }
  ...
  case typevar_constructed_type_id: {
    ...
    if (it->second.get_type_id() == cuda_device_type_id) {
      return ndt::make_cuda_device(
          substitute(pattern.extended<typevar_constructed_type>()->get_arg(),
                     typevars, concrete));
    }
#endif
  }
