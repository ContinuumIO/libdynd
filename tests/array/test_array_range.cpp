...
TEST(ArrayRange, Basic) {
    ...
    a = nd::range(1, 10);
    EXPECT_EQ(ndt::type::make<int32_t>(), a.get_type().get_dtype());
    ...
    a = nd::range(1., 10., 0.5);
    EXPECT_EQ(ndt::type::make<double>(), a.get_type().get_dtype());
    EXPECT_EQ(1u, a.get_shape().size());
    ...
    EXPECT_EQ(ndt::type::make<float>(), a.get_type().get_dtype());
