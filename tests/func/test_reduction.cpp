...
TEST(Reduction, BuiltinSum_Lift0D_WithIdentity)
{
  ...
  EXPECT_ARRAY_EQ(100.0 + 1.25, f(1.25, kwds("identity", 100.0)));
}
*/
...
TEST(Reduction, BuiltinSum_Lift2D_StridedStrided_ReduceReduce_KeepDim)
{
  ...
  EXPECT_ARRAY_EQ(
      ...
      f(initializer_list<initializer_list<double>>{{1.5, 2.0, 7.0}, {-2.25, 7.0, 2.125}}, kwds("keepdims", true)));
}
...
TEST(Reduction, FixedVar_ReduceReduce)
{
  ...
  nd::array a = parse_json("3 * var * float64", "[[1, 2], [3, 4, 5], [6, 7, 8, 9]]");
...
TEST(Reduction, BuiltinSum_Lift2D_VarFixed_ReduceReduce)
{
  ...
  nd::array a = parse_json("var * 2 * float64", "[[1, 2], [3, 4]]");

  EXPECT_ARRAY_EQ(10.0, f(a));
}
