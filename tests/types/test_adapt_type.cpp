...
{
  ndt::type tp =
      ndt::make_type<ndt::adapt_type>(ndt::make_type<float>(), ndt::make_fixed_bytes(sizeof(float), alignof(float)),
...
{
  nd::array a = nd::empty(ndt::make_type<ndt::adapt_type>(ndt::make_type<int16_t>(),
                                                          ndt::make_fixed_bytes(sizeof(int16_t), alignof(int16_t)),
                                                          nd::byteswap::get(), nd::byteswap::get()));
  a.assign(0x1362);
  EXPECT_EQ(0x6213, a.view<int16_t>());

  a = nd::empty(ndt::make_type<ndt::adapt_type>(ndt::make_type<int32_t>(),
                                                ndt::make_fixed_bytes(sizeof(int32_t), alignof(int32_t)),
                                                nd::byteswap::get(), nd::byteswap::get()));
  a.assign(0x12345678);
  EXPECT_EQ(0x78563412, a.view<int32_t>());

  a = nd::empty(ndt::make_type<ndt::adapt_type>(ndt::make_type<int64_t>(),
  ...
  a = nd::empty(ndt::make_type<ndt::adapt_type>(ndt::make_type<float>(),
                                                ndt::make_fixed_bytes(sizeof(float), alignof(float)),
                                                nd::byteswap::get(), nd::byteswap::get()));
  a.assign(alias_cast<float>(0xDA0F4940));
  EXPECT_EQ(3.1415926f, a.view<float>());

  a = nd::empty(ndt::make_type<ndt::adapt_type>(ndt::make_type<double>(),
                                                ndt::make_fixed_bytes(sizeof(double), alignof(double)),
                                                nd::byteswap::get(), nd::byteswap::get()));
  ...
  a = nd::empty(ndt::make_type<ndt::adapt_type>(
      ...
      ndt::make_fixed_bytes(sizeof(dynd::complex<float>), alignof(dynd::complex<float>)), nd::pairwise_byteswap::get(),
