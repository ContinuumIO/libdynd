...
TEST(TypeCasting, IsLosslessAssignment) {
    ...
    EXPECT_FALSE(is_lossless_assignment(ndt::fixed_string_type::make(16, string_encoding_utf_16),
                            ndt::fixed_string_type::make(16, string_encoding_utf_16)));
    EXPECT_FALSE(is_lossless_assignment(ndt::fixed_string_type::make(16, string_encoding_utf_8),
                            ndt::fixed_string_type::make(12, string_encoding_utf_8)));
    EXPECT_FALSE(is_lossless_assignment(ndt::fixed_string_type::make(12, string_encoding_utf_32),
                            ndt::fixed_string_type::make(16, string_encoding_utf_32)));
    EXPECT_FALSE(is_lossless_assignment(ndt::fixed_string_type::make(16, string_encoding_utf_16),
                            ndt::fixed_string_type::make(16, string_encoding_utf_32)));
    EXPECT_FALSE(is_lossless_assignment(ndt::fixed_string_type::make(16, string_encoding_utf_8),
                            ndt::fixed_string_type::make(16, string_encoding_utf_16)));
...
TEST(TypeCasting, StringToInt32) {
    ...
    a.vals() = std::string("2147483647");
    EXPECT_EQ(2147483647, a.as<int>());
    a.vals() = std::string("-2147483648");
    EXPECT_EQ(-2147483647 - 1, a.as<int>());
    EXPECT_THROW(a.vals() = std::string("2147483648"), overflow_error);
    EXPECT_THROW(a.vals() = std::string("-2147483649"), overflow_error);

    // Trailing ".0" is permitted
    a.vals() = std::string("1234.");
    EXPECT_EQ(1234, a.as<int>());
    a.vals() = std::string("2345.0");
    EXPECT_EQ(2345, a.as<int>());
    a.vals() = std::string("3456.00000");
    EXPECT_EQ(3456, a.as<int>());

    // Simple "1e5" positive exponent cases are permitted
    a.vals() = std::string("1e5");
    EXPECT_EQ(100000, a.as<int>());
    a.vals() = std::string("1e9");
    EXPECT_EQ(1000000000, a.as<int>());
    a.vals() = std::string("2e9");
    EXPECT_EQ(2000000000, a.as<int>());
    a.vals() = std::string("-21e8");
    EXPECT_EQ(-2100000000, a.as<int>());
    EXPECT_THROW(a.vals() = std::string("3e9"), overflow_error);
}
...
TEST(TypeCasting, StringToInt64) {
    ...
    a.vals() = std::string("9223372036854775807");
    EXPECT_EQ(9223372036854775807LL, a.as<int64_t>());
    a.vals() = std::string("-9223372036854775808");
    EXPECT_EQ(-9223372036854775807LL - 1LL, a.as<int64_t>());
    EXPECT_THROW(a.vals() = std::string("9223372036854775808"), overflow_error);
    EXPECT_THROW(a.vals() = std::string("-9223372036854775809"), overflow_error);

    // Simple "1e5" positive exponent cases are permitted
    a.vals() = std::string("1e18");
    EXPECT_EQ(1000000000000000000LL, a.as<int64_t>());
    a.vals() = std::string("922e16");
    EXPECT_EQ(9220000000000000000LL, a.as<int64_t>());
    EXPECT_THROW(a.vals() = std::string("1e19"), overflow_error);
}
...
TEST(TypeCasting, StringToInt128) {
    ...
    a.vals() = std::string("-0");
    EXPECT_EQ(0LL, a.as<int64_t>());
    a.vals() = std::string("-170141183460469231731687303715884105728");
    EXPECT_EQ(0x8000000000000000ULL, a.as<int128>().m_hi);
    EXPECT_EQ(0ULL, a.as<int128>().m_lo);
    a.vals() = std::string("170141183460469231731687303715884105727");
    EXPECT_EQ(0x7fffffffffffffffULL, a.as<int128>().m_hi);
    EXPECT_EQ(0xffffffffffffffffULL, a.as<int128>().m_lo);
    EXPECT_THROW(a.vals() = std::string("170141183460469231731687303715884105728"),
                 overflow_error);
    EXPECT_THROW(a.vals() = std::string("-170141183460469231731687303715884105729"),
                 overflow_error);

    // Simple "1e5" positive exponent cases are permitted
    a.vals() = std::string("1e18");
    EXPECT_EQ(1000000000000000000LL, a.as<int64_t>());
    a.vals() = std::string("922e26");
    EXPECT_EQ(0x129ea0d6fULL, a.as<int128>().m_hi);
    EXPECT_EQ(0x287e2f8928000000ULL, a.as<int128>().m_lo);
    EXPECT_THROW(a.vals() = std::string("1e40"), overflow_error);
}
...
TEST(TypeCasting, StringToUInt64) {
    ...
    a.vals() = std::string("-0");
    EXPECT_EQ(0u, a.as<uint64_t>());
    a.vals() = std::string("18446744073709551615");
    EXPECT_EQ(18446744073709551615ULL, a.as<uint64_t>());
    EXPECT_THROW(a.vals() = std::string("18446744073709551616"), overflow_error);
    EXPECT_THROW(a.vals() = std::string("-1"), overflow_error);

    // Simple "1e5" positive exponent cases are permitted
    a.vals() = std::string("1e19");
    EXPECT_EQ(10000000000000000000ULL, a.as<uint64_t>());
    a.vals() = std::string("1844e15");
    EXPECT_EQ(1844000000000000000ULL, a.as<uint64_t>());
    EXPECT_THROW(a.vals() = std::string("1845e20"), overflow_error);
    EXPECT_THROW(a.vals() = std::string("1e20"), overflow_error);
}
